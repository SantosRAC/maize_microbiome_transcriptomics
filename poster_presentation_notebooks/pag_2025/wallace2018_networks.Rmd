 ...
 * The original dataset with all samples (day and night samples considered separately, still selecting samples with RNA-Seq pairs)
...
## SpiecEasi with MB model (separated day and night matrices from the original OTU table after filtering by relative abundance)

Running spiec.easi with the day and night datasets (filtering steps are described in `co_occurrence_matrix_day_periods.ipynb`):

```{r}
library(SpiecEasi)
otu_day_filtered_wallace2018 <- read.csv('/Users/renat/Downloads/tmp/pag_2025/original_otu_filtered_day.tsv', sep='\t', header=TRUE, row.names=1)
otu_night_filtered_wallace2018 <- read.csv('/Users/renat/Downloads/tmp/pag_2025/original_otu_filtered_night.tsv', sep='\t', header=TRUE, row.names=1)
